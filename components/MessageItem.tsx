...
const MessageItem: React.FC<MessageItemProps> = ({ message, onSummarize }) => {
  ...
  // Filter for successfully retrieved URLs to use in summarization
        ...
        <div className={bubbleClasses}>
          ...
          {isModel && message.urlContext && message.urlContext.length > 0 && (
            <div className="mt-3 pt-2.5 border-t border-[rgba(255,255,255,0.1)]">
              <div className="flex items-center justify-between mb-2">
                 ...
                 {successfulUrls.length > 0 && !message.summary && onSummarize && !message.isSummarizing && (
                    <button 
                      ...
                      className="flex items-center gap-1.5 text-[10px] bg-[#79B8FF]/10 text-[#79B8FF] hover:bg-[#79B8FF]/20 px-2 py-1 rounded transition-colors"
                    >
                      <Sparkles size={12} />
                      Summarize Sources
                    </button>
              ...
              {message.summary && (
                <div className="mt-3 p-2.5 bg-[#1E1E1E] rounded-md border border-[rgba(255,255,255,0.05)]">
                   <div className="flex items-center gap-2 mb-1.5">
                      <Sparkles size={14} className="text-[#79B8FF]" />
                      <span className="text-xs font-semibold text-[#E2E2E2]">Source Summary</span>
                   </div>
